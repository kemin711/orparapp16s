...
void usage() {
   cout << "cpfastapart -p 0.5 -o outfile inputfile\n"
      ...
      << "  -r or --repeat INT total number of repeats for same size\n"
      << "      sampling.\n";
   exit(1);
}
...
double GetUniform();
vector<bioseq> buildStore(const string& infile);
void pickSample(const vector<bioseq> &pop, int numseq, const string &outfile,
...
/**
 * repeat picking of the same sample size
 */
void repeatPickSame(const string &inputFile, int samplesz, int numrep);
/**
...
/**
 * copy a fraction of the fasta file into a new file
...
int main(int argc, char* argv[]) {
   ...
   if (!sampleSize.empty()) {
      if (sampleSize.size() > 1) {
         ...
         repeatPickSame(infile, sampleSize[0], numrepeat);
      }
      else {
   ...
   // copy from bebin to fraction of the file
   long begin, end;
...
void repeatPick(const string &inputFile, const vector<int> &samplesz) {
   vector<bioseq> store=buildStore(inputFile);
...
void repeatPickSame(const string &inputFile, int samplesz, int numrep) {
   ...
   for (int i=0; i<numrep; ++i) {
      string outfname = inputFile;
      string::size_type dotpos = inputFile.rfind('.');
      string extra = "_" + to_string(samplesz) + "_r" + to_string(i);
      if (dotpos != string::npos) {
         outfname.insert(dotpos, extra) ;
      }
      else outfname += extra + ".fas";
      pickSample(store, samplesz, outfname);
   }
}
...
vector<bioseq> buildStore(const string& infile) {
   ...
   vector<bioseq> store;
   bioseq fasta;
   while (fasta.read(inf)) {
      store.push_back(fasta);
   }
   return store;
}

/**
 * sample numseq sequences from fasta input pop
 *
